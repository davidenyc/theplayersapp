        ...
        <div className="flex gap-2">
          {[7, 30].map((option) => (
            <button
              key={option}
              type="button"
              onClick={() => setWindow(option as 7 | 30)}
              className={`rounded-full px-3 py-1 text-sm font-medium ${
                window === option ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"
              }`}
            >
              {option}d
            </button>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={view}>
            ...
            {flagThreshold !== undefined ? (
              <ReferenceLine y={flagThreshold} stroke="#ef4444" strokeDasharray="5 5" />
            ) : null}
            <Line
              type="monotone"
              dataKey="value"
